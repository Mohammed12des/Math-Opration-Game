...
let clock = new Audio("sound/clock.mp3");
let millon = new Audio("sound/million.mp3");
let win = new Audio("sound/win.wav");
let loss = new Audio("sound/loss.mp3");
let correct = new Audio("sound/coorect.mp3");
let wrong = new Audio("sound/Time.wav");
let timer;
let num1;
let num2;
let num3;
...
enter.addEventListener("click", () => {
  if (c === numberQueestion) {
    ...
    tital.innerText = "WOW, You Win, Your level is not here go up";
    clearInterval(timer);
    score.innerText = `You Answer All Questions in ${time - remain} second`;
    enter.disabled = true;
  } else {
    if (
      parseInt(sloution.value) === slove &&
      c <= numberQueestion &&
      easy === true
    ) {
      q++;
